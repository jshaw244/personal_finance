...
.DESCRIPTION
    ...
    - Checks for other running Flask environments (sandbox/dev/prod)
    - Starts Flask app (src\ingestion\app.py)
    - Launches schema watcher in separate PowerShell window
    - Sets custom window titles
...
$EnvFile       = ".\config\env\.env.development"
...
function Write-Log {
    ...
    Add-Content -Path $LogFile -Value "[$ts] DEV_RUN - $Message"
    Write-Host $Message
}
...
if (-not (Test-Path $EnvFile)) {
    Write-Log "ERROR: .env.development not found: $EnvFile"
    exit 1
}

# --- Step 2: Load environment variables ---
Write-Log "Loading .env.development..."
$envVars = Get-Content $EnvFile | Where-Object { $_ -match "=" -and $_ -notmatch "^#" }
foreach ($line in $envVars) {
    $name, $value = $line -split "=", 2
    ${env:$name} = $value
}
${env:ENV_TARGET} = "development"
...
if ($existingWatcher) {
    ...
    Write-Log "Launching schema watcher in separate PowerShell window..."
    $watcherTitle = "personal_finance [DEV] - Schema Watcher"
    $watcherCommand = "[Console]::Title = '$watcherTitle'; `$Host.UI.RawUI.WindowTitle = '$watcherTitle'; Write-Host 'Schema watcher active (auto-sync enabled).'; while (`$true) { Start-Sleep 30 }"
    Start-Process pwsh -ArgumentList "-NoExit", "-Command", $watcherCommand
    Write-Log "Schema watcher launched."
}

# --- Step 5: Start Flask app ---
Write-Log "Starting Flask development app..."
$flaskTitle = "personal_finance [DEV] - Flask App"
