...
if (-not $env:PLAID_CLIENT_ID -or -not $env:PLAID_SECRET) {
    Write-Host "âŒ Missing PLAID_CLIENT_ID or PLAID_SECRET in environment. Check your .env.sandbox file." -ForegroundColor Red
    exit 1
}
...
if (-not $itemsJson) {
    Write-Host "âŒ No items found in DB. Connect banks first." -ForegroundColor Red
    exit 1
}
...
foreach ($item in $items) {
    ...
    Write-Host "âž¡ï¸  Creating transactions for ${institution} ..." -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host "âœ… ${institution}: Created $($resp.transactions.Count) transactions" -ForegroundColor Green
    }
    catch {
        Write-Host "âŒ ${institution}: Error firing transactions" -ForegroundColor Red
